...
build:
	...
	@$(DOCKER) build --build-arg CERT_MANAGER_COMPONENT=acmesolver \
		-t $(REGISTRY_IMAGE)-acmesolver:v${VERSION}-$(arch) $(version) -f $(version)/Dockerfile.$(arch)
	@$(DOCKER) build --build-arg CERT_MANAGER_COMPONENT=controller \
		-t $(REGISTRY_IMAGE)-controller:v${VERSION}-$(arch) $(version) -f $(version)/Dockerfile.$(arch)
...
publish:
	...
	@$(DOCKER) push $(REGISTRY_IMAGE)-controller:v$(VERSION)-$(arch)
	@$(DOCKER) push $(REGISTRY_IMAGE)-acmesolver:v$(VERSION)-$(arch)
